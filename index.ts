...
function reportHtml(r: any) {
  ...
  return `<html><body style="font-family:Arial,sans-serif;max-width:760px;margin:auto;padding:20px;color:#111827"><h1>🏈 Family NFL Picks — Week ${r.week}</h1><p>${r.season} season · ${esc(r.round)} · ${r.overrideCount||0} commissioner overrides</p><h2>Weekly results</h2><table width="100%" cellpadding="7"><tr><th align="left">Player</th><th align="right">Checks</th><th align="right">Week points</th><th align="right">Season total</th></tr>${weekly}</table><h2>Every game</h2>${games}<h2>Leaderboard</h2><table width="100%" cellpadding="7"><tr><th align="left">#</th><th align="left">Player</th><th align="right">Total</th></tr>${leaders}</table></body></html>`;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!Number.isInteger(season) || !Number.isInteger(week) || week < 1 || week > 30 || !Array.isArray(payload?.games) || !Array.isArray(payload?.leaderboard)) {
    ...
    const from=Deno.env.get('REPORT_FROM') || 'Family NFL Picks <onboarding@resend.dev>';
    ...
    await supabase.from('weekly_report_delivery').upsert({season,week,payload},{onConflict:'season,week'});

    const rr=await fetch('https://api.resend.com/emails',{
      method:'POST',
      headers:{'authorization':`Bearer ${resendKey}`,'content-type':'application/json'},
      body:JSON.stringify({from,to:[recipient],subject:`Family NFL Picks — Week ${week} Report`,html:reportHtml(payload)})
    });
    const out=await rr.json();
    if(!rr.ok) throw new Error(out?.message||`Resend HTTP ${rr.status}`);

...
const KEY='family-nfl-picks-v01'; // keep the same storage key so v0.1 test data carries forward
const DEMO_GAMES=[
...
function baseState(){return {
  season:2026,currentWeek:1,currentUser:null,screen:'home',historyWeek:null,scheduleSource:'demo',
...
async function tryLoadSchedule(){
  ...
  try{
    ...
    const games=d.events.map(e=>{const c=e.competitions?.[0];const comps=c?.competitors||[];const h=comps.find(x=>x.homeAway==='home');const a=comps.find(x=>x.homeAway==='away');return {
      ...
      kickoff:e.date,status:e.status?.type?.state||'scheduled',round:'regular',homeScore:Number(h?.score),awayScore:Number(a?.score)
    }});
...
function checkCountsForWeek(w){
 ...
 for(const g of w.games||[]){const r=w.results?.[g.id]; if(!r) continue; const checks=resolveGameChecks({picksByUser:w.picks?.[g.id]||{},homeTeam:g.home,awayTeam:g.away,homeScore:r.homeScore,awayScore:r.awayScore}); for(const p of PLAYERS) if(checks[p.id]) counts[p.id]++}
 return counts;
}
...
function bind(){
 ...
 document.querySelectorAll('[data-act]').forEach(b=>b.onclick=async()=>{
   ...
   if(a==='score-week'){if(!user().admin)return;if(wk().scoredAt)return alert('This week is already scored. It will not be added to the season total twice.');const counts=weeklyCheckCounts();const mult=playoffRoundMultiplier(wk().round);const pts=normalizeChecks(counts,mult);PLAYERS.forEach(p=>state.totals[p.id]=(state.totals[p.id]||0)+pts[p.id]);const at=new Date().toISOString();wk().scoredAt=at;wk().scoreSummary={checks:counts,points:pts,multiplier:mult,scoredAt:at};state.historyWeek=String(state.currentWeek);save();alert('Week scored, added to season totals, and saved to History.');render()}
